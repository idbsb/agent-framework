...
export default function JobAnalysisPage() {
  ...
  useEffect(() => {
    ...
    getJson<{ jobs: Analysis[] }>("/data/job_analysis_v1.json?v=job-profile-v4").then((result) => setFallbackData(result.data.jobs)).catch(() => setFallbackData([]));
  }, []);
  useEffect(() => { setMessage(""); getJson<Analysis>(`/api/job-analysis/${encodeURIComponent(title)}`).then((result) => setData(result.data)).catch(() => { const cached = fallbackData.find((item) => item.job_title === title); if (cached) setData(cached); else { setData(null); setMessage("岗位数据加载失败，请刷新页面重试。"); } }); }, [title, fallbackData]);
...
function Info({ label, value }: { label: string; value: string }) { return <div className="info-row"><span>{label}</span><p>{value || "招聘信息未明确"}</p></div>; }
