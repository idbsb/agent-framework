...
import { QualitySummary, safeEvolution, type Quality, type WindowSamples } from "../components/DataQuality";

type Evolution = { data_quality?: Quality; window_samples?: WindowSamples; available: boolean; status: string; job_title: string; message: string; notice: string; time_range: string[] | null; support_jd_count: number; sample_insufficient: boolean; sample_notice: string; core_skills: string[]; growing_skills: string[]; new_skills: string[]; stable_skills: string[]; declining_skills: string[]; sample_insufficient_skills: string[] };
...
export default function EvolutionPage() {
  ...
  useEffect(() => { getJson<EvolutionSource>(`/api/evolution/job/${encodeURIComponent(title)}`, "/data/evolution_status.json").then((result) => { const value = "jobs" in result.data ? result.data.jobs[title] : result.data; setData(value ? safeEvolution({ ...value, job_title: title }, result.fallback) : null); setFallback(result.fallback); }); }, [title]);
  return <><PageIntro kicker="近期观察 · 时间范围 · 样本约束" title="岗位技能动态演化" description="只展示组员A正式演化结果；不根据当前数据自行生成趋势。" index="04" />
    ...
    <QualitySummary data={data?.data_quality} windows={data?.window_samples} />
