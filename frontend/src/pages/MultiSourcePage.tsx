...
export default function MultiSourcePage() {
  ...
  const cards = [
    [data?.current.jd_count, "当前观察样本", Database], [data?.incremental.source_count, "招聘来源平台", RadioTower],
    [data?.incremental.company_count, "覆盖招聘企业", Building2], [data?.incremental.dated_jd_count, "有发布日期JD", CheckCircle2],
    [data?.external_source_counts["政策文件"], "政策佐证", FileText], [data?.external_source_counts["行业报告"], "行业报告佐证", FileCheck2],
  ] as const;
  return <><PageIntro kicker="招聘数据源 · 外部佐证 · 真实性审计" title="多源数据采集与质量追溯" description="招聘JD用于数量与岗位变化计算；政策和行业报告仅作为外部趋势佐证，两类口径严格分开。" />
    ...
    <StatusBanner>多源证据口径：招聘JD形成岗位与能力结论，政策和行业报告仅用于交叉佐证，所有结果均可追溯。</StatusBanner>
