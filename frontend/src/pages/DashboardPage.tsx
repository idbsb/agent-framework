...
export default function DashboardPage() {
  ...
  return <>
    ...
    <section className="hero-panel"><div><p className="kicker">真实招聘数据 · 招聘市场变化驱动 · 动态岗位能力图谱</p><h2>{data?.truth_statement || "正在读取岗位与技能数据…"}</h2><p>多源招聘数据驱动新岗位发现与既有岗位能力更新，并将最新 Graph V2 用于简历解析、精准匹配和差距分析。</p></div></section>
    <section className="closure-flow"><Link to="/multi-source">多源招聘数据</Link><span>→</span><Link to="/job-changes">新岗位发现 / 能力更新</Link><span>→</span><Link to="/graph">动态能力图谱</Link><span>→</span><Link to="/resume-parse">简历解析</Link><span>→</span><Link to="/match">精准匹配</Link><span>→</span><Link to="/gap-analysis">差距分析</Link></section>
    ...
    <section className="dashboard-grid">
      <article className="panel batch-panel"><div className="panel-title"><span>最近一次图谱更新</span><small>{data?.batch_update.batch_id || "加载中"}</small></div><div className="batch-stats"><span><b>+{data?.batch_update.incremental.jd_count ?? 0}</b>新增JD</span><span><b>+{data?.batch_update.incremental.emerging_candidate_count ?? 0}</b>新岗位候选</span><span><b>+{data?.batch_update.incremental.changed_job_count ?? 0}</b>能力更新岗位</span><span><b>{data?.batch_update.graph_version || "—"}</b>当前图谱</span></div><p>最近更新时间：{data?.batch_update.updated_at || "—"}</p><Link className="text-link" to="/job-changes">查看本次变化 →</Link></article>
