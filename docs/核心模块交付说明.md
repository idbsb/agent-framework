...
依赖记录在`requirements.txt`：openpyxl、PyYAML、Pydantic、FastAPI、Uvicorn、python-multipart、pypdf、python-docx。
...
简历文件入口使用 `POST /api/resume/extract`，支持 PDF、DOCX、TXT（最大 8MB、仅内存解析），提取结果先回填表单供用户确认，再调用既有 `POST /api/resume/parse`。
