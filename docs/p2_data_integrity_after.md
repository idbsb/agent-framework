...
日期：2026-08-31。与 p2_data_integrity_before.md 同范围逐项比较：**63/63 路径、字节数、SHA-256 完全一致；0 个原始数据文件修改或删除。**

旧业务/参考数据与旧验收数据库均保持不变。扫描发现的新文件全部为本次明确的派生验收结果（下列列表），不属于新增业务数据。tests/fixtures/p2_synthetic 内明确标记的测试数据单独管理；测试数据库只放临时目录，不进入 Git。
...
- `.codex_artifacts/p2/system-qa-20260831/system_qa_results_graph_dynamic_v2.json`
- `.codex_artifacts/p2/system-qa-final-20260831/system_qa_results_graph_dynamic_v2.json`

以下为受保护的原始 63 个文件完成后的实测值：
